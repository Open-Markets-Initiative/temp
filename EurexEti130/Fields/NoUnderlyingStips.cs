using System.Runtime.CompilerServices;

namespace Eurex.Eti.v130;

/// <summary>
///  No Underlying Stips: Runtime Count Field
/// </summary>

public static class NoUnderlyingStips
{
    /// <summary>
    ///  Fix Tag for No Underlying Stips
    /// </summary>
    public const ushort FixTag = 887;

    /// <summary>
    ///  Length of No Underlying Stips in bytes
    /// </summary>
    public const int Length = 1;

    /// <summary>
    ///  Encode No Underlying Stips
    /// </summary>
    public unsafe static void Encode(byte* pointer, int offset, byte value, int length, out int current)
    {
        if (length > offset + NoUnderlyingStips.Length)
        {
            throw new System.Exception("Invalid Length for No Underlying Stips");
        }

        Encode(pointer, offset, value, out current);
    }

    /// <summary>
    ///  Encode No Underlying Stips
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public unsafe static void Encode(byte* pointer, int offset, byte value, out int current)
    {
        Encode(pointer, offset, value);

        current = offset + NoUnderlyingStips.Length;
    }

    /// <summary>
    ///  Encode No Underlying Stips
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public unsafe static void Encode(byte* pointer, int offset, byte value)
    {
        *(byte*) (pointer + offset) = value;
    }

    /// <summary>
    ///  Check available length and set No Underlying Stips to 0
    /// </summary>
    public unsafe static void Zero(byte* pointer, int offset, int length, out int current)
    {
        if (length > offset + NoUnderlyingStips.Length)
        {
            throw new System.Exception("Invalid Length for No Underlying Stips");
        }

        Zero(pointer, offset, out current);
    }

    /// <summary>
    ///  Set No Underlying Stips to no value and update index
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public unsafe static void Zero(byte* pointer, int offset, out int current)
    {
        Zero(pointer, offset);

        current = offset + NoUnderlyingStips.Length;
    }

    /// <summary>
    ///  Set No Underlying Stips to 0
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public unsafe static void Zero(byte* pointer, int offset)
    {
        *(byte*) (pointer + offset) = 0;
    }

    /// <summary>
    ///  TryDecode No Underlying Stips
    /// </summary>
    public unsafe static bool TryDecode(byte* pointer, int offset, int length, out byte value, out int current)
    {
        if (length > offset + NoUnderlyingStips.Length)
        {
            return TryDecode(pointer, offset, out value, out current);
        }

        value = default;

        current = offset;

        return false;
    }

    /// <summary>
    ///  TryDecode No Underlying Stips
    /// </summary>
    public unsafe static bool TryDecode(byte* pointer, int offset, out byte value, out int current)
    {
        value = Decode(pointer, offset, out current);

        return value > 0;
    }

    /// <summary>
    ///  Decode No Underlying Stips
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public unsafe static byte Decode(byte* pointer, int offset, out int current)
    {
        current = offset + NoUnderlyingStips.Length;

        return Decode(pointer, offset);
    }

    /// <summary>
    ///  Decode No Underlying Stips
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public unsafe static byte Decode(byte* pointer, int offset)
    {
        return *(byte*) (pointer + offset);
    }
}
